...
if __name__ == '__main__':
    ...
    n = 1000
    x = np.random.randn(n)
    x = np.exp(x)  # Make it non-Gaussian
    x = x + 0.1 * np.random.randn(n)  # Add some noise

    transformer = Gaussianize(strategy='lambert', verbose=True)
    transformer.fit(x)
    y = transformer.transform(x)
    x_rec = transformer.inverse_transform(y)

    print("Original: mu=%.4f, sigma=%.4f, skew=%.4f, kurt=%.4f" % (np.mean(x), np.std(x), stats.skew(x), stats.kurtosis(x, fisher=True)))
    print("Gaussianized: mu=%.4f, sigma=%.4f, skew=%.4f, kurt=%.4f" % (np.mean(y), np.std(y), stats.skew(y), stats.kurtosis(y, fisher=True)))
    print("Recovered: mu=%.4f, sigma=%.4f, skew=%.4f, kurt=%.4f" % (np.mean(x_rec), np.std(x_rec), stats.skew(x_rec), stats.kurtosis(x_rec, fisher=True)))
    ...
    plt.figure()
    plt.hist(y, bins=30)
    plt.title('Gaussianized')
    plt.figure()
    plt.hist(x_rec, bins=30)
    plt.title('Recovered')
    plt.show()